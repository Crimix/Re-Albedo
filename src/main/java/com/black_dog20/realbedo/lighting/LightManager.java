...
import com.black_dog20.realbedo.ReAlbedo;
import com.black_dog20.realbedo.event.GatherLightsEvent;
import com.black_dog20.realbedo.util.ShaderManager;
...
public class LightManager {
    ...
    public static ClippingHelper camera;
    ...
    public static void uploadLights() {
        ...
        for (int i = 0; i < Math.min(Config.maxLights.get(), lights.size()); i++) {
            if (i < lights.size()) {
    ...
    private static Vector3d interpolate(Entity entity, float partialTicks) {
        return new Vector3d(
    ...
    public static void update(World world) {
        ...
        if (cameraEntity != null && camera != null) {
            cameraPos = interpolate(cameraEntity, mc.getRenderPartialTicks());
            camera = new ClippingHelper(m, m);
        ...
        GatherLightsEvent event = new GatherLightsEvent(lights, Config.maxDistance.get(), cameraPos, camera);
        MinecraftForge.EVENT_BUS.post(event);

        int maxDist = Config.maxDistance.get();
        ...
        for (Entity e : world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(
            ...
            if (e instanceof ItemEntity) {
                LazyOptional<ILightProvider> provider = ((ItemEntity) e).getItem().getCapability(ReAlbedo.LIGHT_PROVIDER_CAPABILITY);
                provider.ifPresent(p -> p.gatherLights(event, e));
            }
            LazyOptional<ILightProvider> provider = e.getCapability(ReAlbedo.LIGHT_PROVIDER_CAPABILITY);
            provider.ifPresent(p -> p.gatherLights(event, e));
            for (ItemStack itemStack : e.getHeldEquipment()) {
                provider = itemStack.getCapability(ReAlbedo.LIGHT_PROVIDER_CAPABILITY);
                provider.ifPresent(p -> p.gatherLights(event, e));
            }
        ...
        for (TileEntity t : world.loadedTileEntityList) {
            LazyOptional<ILightProvider> provider = t.getCapability(ReAlbedo.LIGHT_PROVIDER_CAPABILITY);
