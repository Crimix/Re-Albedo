...
import com.black_dog20.realbedo.event.LightUniformEvent;
...
import com.black_dog20.realbedo.lighting.LightManager;
...
public class EventManager {
            ...
            while (!thread.isInterrupted()) {
                if (Minecraft.getInstance().player != null) {
                    ...
                    if (Minecraft.getInstance().world != null) {
                        ...
                        int maxDistance = Config.maxDistance.get();
                        int r = maxDistance / 2;
                        ...
                        for (BlockPos pos : posIterable) {
                            Vector3d cameraPosition = LightManager.cameraPos;
                            ClippingHelper camera = LightManager.camera;
                            BlockState state = reader.getBlockState(pos);
                            ArrayList<Light> lights = new ArrayList<>();
                            GatherLightsEvent lightsEvent = new GatherLightsEvent(lights, maxDistance, cameraPosition, camera);
                            TriConsumer<BlockPos, BlockState, GatherLightsEvent> consumer = ReAlbedo.MAP.get(state.getBlock());
                            if (consumer != null)
        ...
        if (Config.isLightingEnabled()) {
            ...
            if (event.getSection().compareTo("entities") == 0) {
                if (Minecraft.getInstance().isOnExecutionThread()) {
                    ...
                    ShaderUtil.entityLightProgram.setUniform("fogIntensity", Minecraft.getInstance().world.func_230315_m_().func_242725_p().equals(DimensionType.field_242711_b) ? 0.015625f : 1.0f);
                }
            }
    ...
    /* Until I make Chuck renders work again by finding out what preRenderChunk in ChunkRenderContainer is
    @SubscribeEvent
    public void onRenderChunk(RenderChunkUniformsEvent event) {
        if (Config.isLightingEnabled()) {
            if (ShaderManager.isCurrentShader(ShaderUtil.fastLightProgram)) {
                BlockPos pos = event.getChunk().getPos().asBlockPos();
    ...
    public void attachCapabilities(AttachCapabilitiesEvent<ItemStack> event) {
        if (Config.enableTorchImplementation.get()) {
            if (event.getObject().getItem() == Blocks.TORCH.asItem()) {
                event.addCapability(new ResourceLocation("albedo", "light_provider"), new ICapabilityProvider() {
                    ...
                    public <T> LazyOptional<T> getCapability(@Nonnull Capability<T> cap, @Nullable Direction side) {
                        if (cap == ReAlbedo.LIGHT_PROVIDER_CAPABILITY)
